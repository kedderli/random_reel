...
const colorsSet = ['#e6b996', '#4626ac', '#9d4778', '#b567b7', '#272151'];
...
export const drawWheel = (ctx: CanvasRenderingContext2D, options: Option[]) => {
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    ctx.fillStyle = 'white';
    ctx.translate(
      convasCenter + Math.cos(angle + arc / 2) * textRadius,
      convasCenter + Math.sin(angle + arc / 2) * textRadius,
    );
    ctx.rotate(angle + arc / 2 + Math.PI / 2 - 1.65);
